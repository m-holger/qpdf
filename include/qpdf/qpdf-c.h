...
#   define QPDF_FALSE 0
     ...
     *
     * There are (only) two distinct reasons why warnings are generated:
     *
     * 1. an unresolved indirect QPDFObjectHandle cannot be resolved
     *
     *    This can only happen the very first time an object is
     *    accessed.
     *
     * 2. a function is called with an inappropriate object, e.g.
     *    qpdf_oh_get_bool_value is called with an array object.
     *
     * No other warnings are generated by these functions.
     *
     * If a function call generates a warning, the function will return
     * a default value depending on the return type:
     *
     * Return type  | Default
     * -------------|------------
     * qpdf_oh      | Null object
     ...
     * char const*  | ""
     *
     * If any other value is returned by these functions there will be
     * no need to check for warnings.
     *
     * qpdf_get_trailer and qpdf_get_root will throw unhandled exceptions
     * if they encounter problems, e.g. if they are called with an
     * improperly initialised qpdf_data object.
     */
