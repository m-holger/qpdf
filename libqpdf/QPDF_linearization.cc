...
QPDF::readHintStream(Pipeline& pl, qpdf_offset_t offset, size_t length)
{
    ...
    if (length_obj.isIndirect()) {
        ...
        Obj& oc2 = m->obj_table[length_obj.getObjGen()];
...
QPDF::maxEnd(ObjUser const& ou)
{
    ...
    for (auto const& og: m->obj_user_to_objects[ou]) {
        ...
        end = std::max(end, m->obj_table[og].end_after_space);
    }
    return end;
}
...
{
    QPDFXRefEntry entry = m->obj_table[og].xref;
...
QPDF::lengthNextN(int first_object, int n)
{
    ...
    auto iter = m->obj_table.find(QPDFObjGen(first_object, 0));
    auto end = m->obj_table.end();
    for (int i = 0; i < n; ++i) {
        QPDFObjGen og(first_object + i, 0);
        if (iter == end || iter->first != og) {
            stopOnError("found unknown object while calculating length for linearization data");
        }
        if (!iter->second.contains_xref()) {
            linearizationWarning("no xref table entry for " + og.unparse(' '));
        } else {
            length += iter->second.end_after_space - getLinearizationOffset(og);
        }
        ++iter;
    }
    return toI(length);
    ...
    std::vector<QPDFObjectHandle> const& pages, std::map<int, int>& shared_idx_to_obj)
{
    ...
    if (!m->obj_table.contains_xref(first_page_og)) {
    ...
    for (int pageno = 0; pageno < npages; ++pageno) {
        ...
        if (!m->obj_table.contains_xref(page_og)) {
    ...
    if (so.nshared_total < so.nshared_first_page) {
        ...
        for (int i = 0; i < so.nshared_total; ++i) {
            if (i == so.nshared_first_page) {
                ...
                if (!m->obj_table.contains_xref(og)) {
