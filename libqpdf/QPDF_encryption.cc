...
QPDF::initializeEncryption()
{
    ...
    if (!m->objects.xref_table().trailer().hasKey("/Encrypt")) {
        return;
    }
    ...
    std::string id1;
    QPDFObjectHandle id_obj = m->objects.xref_table().trailer().getKey("/ID");
    ...
    QPDFObjectHandle encryption_dict = m->objects.xref_table().trailer().getKey("/Encrypt");
