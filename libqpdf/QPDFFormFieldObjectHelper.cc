...
FormNode::generateTextAppearance(QPDFAnnotationObjectHelper& aoh)
{
    ...
    if (!AS) {
        // This can only have aoh.getAppearanceStream("/N") did not return a stream. The only way
        // creation of a new AS could have failed is if getOwningQPDF returned a nullptr, but this
        // would throw a runtime error in newStream. So this should be impossible
