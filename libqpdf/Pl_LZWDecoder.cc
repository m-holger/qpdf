...
void
Pl_LZWDecoder::addToTable(unsigned char next_char)
{
...
Pl_LZWDecoder::handleCode(unsigned int code)
{
    ...
    if (code == 256) {
        ...
        if (this->last_code != 256) {
            ...
            if (code < 256) {
                // just read < 256; last time's next_char was code
                next_char = static_cast<unsigned char>(code);
            } else if (code > 257) {
                size_t idx = code - 258;
                if (idx > table_size) {
                    ...
                    next_char = getFirstChar(this->last_code);
                } else {
                    next_char = getFirstChar(code);
                }
            }
